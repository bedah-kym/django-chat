...
class Chatroom(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.encryption_key:
            ...
            raw_key = b64encode(key).decode('utf-8')
            self.encryption_key = f"enc:{TokenEncryption.encrypt(raw_key)}"
        elif self.encryption_key and not self.encryption_key.startswith("enc:"):
            try:
                self.encryption_key = f"enc:{TokenEncryption.encrypt(self.encryption_key)}"
            except Exception:
                pass
