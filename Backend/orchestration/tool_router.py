"""
Tool Router (formerly mcp_router.py — renamed in v0.5).
...
and was a source of confusion for OSS readers. `mcp_router.py` remains as
a re-export shim for one cycle for external importers; new code imports
from `orchestration.tool_router`.

The inline connectors defined below (CalendarConnector, SearchConnector,
WeatherConnector, GiphyConnector, CurrencyConnector, ReminderConnector)
are still slated to move into per-file modules under
`Backend/orchestration/connectors/` — see the v0.4 M2-3 note.
"""
import json
import logging
import threading
